...
from claspy.window_multi import map_window_size_methods_multi
...
class BinaryClaSPSegmentation:
    ...
    use_dims : int, default= -1
        Number of relevant dimensions for the knn calculation. 
    Methods
    -------
    fit(time_series)
    ...
    def __init__(self, n_segments="learn", n_estimators=10, window_size="suss", k_neighbours=3,
                 ...
                 n_jobs=-1, random_state=2357, use_dims=-1):
    ...
    def _local_segmentation(self, lbound, ubound, change_points):
        ...
        split = np.split(self.time_series, [lbound, ubound], axis = 1)
        ...
        ).fit(split[1], validation=self.validation, threshold=self.threshold)
    ...
    def fit(self, time_series):
        ...
        """
        #check_input_time_series(time_series)  # check if all dim equal length??
        
        #if user tries to use more dimensions than TS has
        if self.use_dims > time_series.shape[0]:
            raise ValueError(f"{self.use_dims} is not a valid number of dimensions. use_dims need to be in range: 1,{time_series.shape[0]}")
        
        if len(time_series.shape) == 1:
            time_series = np.array([time_series],dtype=np.float64)

        self.window_size = map_window_size_methods_multi(self.window_size)(time_series)

        self.min_seg_size = np.max(self.window_size) * self.excl_radius  
        ts_points = time_series.shape[1]

        if ts_points < 2 * self.min_seg_size:
            warnings.warn(
        ...
        self.n_timepoints = ts_points

        self.queue = PriorityQueue()
        self.clasp_tree = []

        if self.n_segments == "learn":
            self.n_segments = ts_points // self.min_seg_size
        ...
        if self.n_segments > 1:
            ...
            profile = np.full(shape=self.n_timepoints - np.max(self.window_size) + 1, fill_value=-np.inf, dtype=np.float64)
        ...
        for idx in range(self.n_segments - 1):
            ...
            profile[lbound:ubound - np.max(self.window_size) + 1] = np.max( 
                [profile[lbound:ubound - np.max(self.window_size) + 1], clasp.profile], axis=0) 
    ...
    def predict(self, sparse=True):
        ...
        #hier muss das vielleicht noch angepassst werden -> geht bei multi dim noch nicht !!
        seg_idx = np.concatenate(([0], self.change_points, [self.time_series.shape[1]]))
    ...
    def plot(self, gt_cps=None, heading=None, ts_name=None, fig_size=(20, 18), font_size=22, file_path=None):
        """
        ...
        """
        self._check_is_fitted()
        fig, ax = plt.subplots(self.time_series.shape[0]+1, sharex=True, gridspec_kw={"hspace": .05}, figsize=fig_size) 

        ts_dim = self.time_series.shape[0]
        if gt_cps is not None:
            segments = [0] + gt_cps.tolist() + [self.time_series.shape[1]]
            for dim in range(ts_dim):
                for idx in np.arange(0, len(segments) - 1):
                    ax[dim].plot(np.arange(segments[idx], segments[idx + 1]), self.time_series[dim][segments[idx]:segments[idx + 1]])
            ax[ts_dim].plot(np.arange(self.profile.shape[0]), self.profile, color="black")
        else:
            for dim in range(ts_dim):
                ax[dim].plot(np.arange(self.time_series.shape[1]), self.time_series[dim])
            ax[ts_dim].plot(np.arange(self.profile.shape[0]), self.profile, color="black")

        if heading is not None:
            ax[0].set_title(heading, fontsize=font_size)

        if ts_name is not None:
            for dim in range(ts_dim):
                ax[dim].set_ylabel(ts_name[dim], fontsize=font_size)
        
        ax[ts_dim].set_xlabel("split point", fontsize=font_size)
        ax[ts_dim].set_ylabel("ClaSP Score", fontsize=font_size)

        for a in range(ts_dim+1):
            for tick in ax[a].xaxis.get_major_ticks():
                tick.label1.set_fontsize(font_size)

            for tick in ax[a].yaxis.get_major_ticks():
                tick.label1.set_fontsize(font_size)

        if gt_cps is not None:
            for idx, true_cp in enumerate(gt_cps):
                for dim in range(ts_dim+1):
                    ax[dim].axvline(x=true_cp, linewidth=2, color="r", label="True Change Point" if idx == 0 else None)

        for idx, found_cp in enumerate(self.change_points):
            for dim in range(ts_dim+1):
                ax[dim].axvline(x=found_cp, linewidth=2, color="g", label="Predicted Change Point" if idx == 0 else None)
