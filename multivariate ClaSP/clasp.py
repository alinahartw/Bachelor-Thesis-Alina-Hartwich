...
class ClaSP:
    ...
    def __init__(self, window_size=10, k_neighbours=3, distance="znormed_euclidean_distance", score="roc_auc",
                 excl_radius=5, n_jobs=-1, use_dims = -1):
    ...
    def fit(self, time_series, knn=None):
        ...
        self.lbound, self.ubound = 0, time_series.shape[1]

        if time_series.shape[1] < 2 * self.min_seg_size:
        ...
        for idx in range(n_jobs):
            ...
            end = min((idx + 1) * bin_size, self.knn.offsets.shape[0] - self.min_seg_size + np.max(self.window_size)) 
    ...
    def split(self, sparse=True, validation="significance_test", threshold=1e-15):
        ...
        split = np.split(self.time_series, [cp], axis = 1) 
...
class ClaSPEnsemble(ClaSP):
    ...
    use_dims : int, optional
        Number of dimensions used for knn calculation, defalut = -1 (algorithm calculates the relevant dimensions by itself)
    Methods
    -------
    fit(time_series)
    ...
    def __init__(self, n_estimators=10, window_size=10, k_neighbours=3, distance="znormed_euclidean_distance",
                 score="roc_auc", early_stopping=True, excl_radius=5, n_jobs=-1, random_state=2357, use_dims = -1):
        super().__init__(window_size, k_neighbours, distance, score, excl_radius, n_jobs, use_dims)
        self.use_dims = use_dims
    ...
    def _calculate_temporal_constraints(self):
        ...
        ts_points = self.time_series.shape[1]
        tcs = [(0, ts_points)]

        np.random.seed(self.random_state)

        while len(tcs) < self.n_estimators and ts_points > 3 * self.min_seg_size:
            lbound, area = np.random.choice(ts_points, 2, replace=True)
            if ts_points - lbound < area:
                area = ts_points - lbound
    ...
    def fit(self, time_series, knn=None, validation="significance_test", threshold=1e-15):
        ...
        for idx, (lbound, ubound) in enumerate(tcs):
            ...
            split = np.split(time_series, [lbound, ubound], axis = 1)
            ...
            clasp = ClaSP(
                ...
                n_jobs=self.n_jobs
            ).fit(split[1], knn=knn.constrain(lbound, ubound))

            clasp.profile = (clasp.profile + (ubound - lbound) / ts_points) / 2
        ...
        self.profile = np.full(shape=ts_points - np.max(self.window_size) + 1, fill_value=-np.inf, dtype=np.float64) 
        self.profile[self.lbound:self.ubound - np.max(self.window_size) + 1] = best_clasp.profile               
